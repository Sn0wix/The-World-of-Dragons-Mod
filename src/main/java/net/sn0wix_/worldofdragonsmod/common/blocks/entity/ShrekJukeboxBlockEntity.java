...
import net.sn0wix_.worldofdragonsmod.common.blocks.custom.ShrekJukeboxBlock;
import net.sn0wix_.worldofdragonsmod.common.item.ModItems;
import org.jetbrains.annotations.Nullable;

import java.util.Objects;

public class ShrekJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory {
    ...
    public ShrekJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.SHREK_JUKEBOX_ENTITY, pos, state);
    }
    ...
    private void updateState(@Nullable Entity entity, boolean hasRecord) {
        if (this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), this.getCachedState().with(ShrekJukeboxBlock.HAS_RECORD, hasRecord), Block.NOTIFY_LISTENERS);
    ...
    public void setStack(int slot, ItemStack stack) {
        if (stack.isIn(ItemTags.MUSIC_DISCS) && this.world != null && stack.getItem().equals(ModItems.MUSIC_DISC_SHREK)) {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, ShrekJukeboxBlockEntity blockEntity) {
